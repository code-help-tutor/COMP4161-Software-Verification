...
typedef lets us define a new type as a subset of an existing type (set).
Here, the existing set is ('a \<rightharpoonup> 'b) set, whose element is a map from 'a to 'b (i.e., a function
from 'a to 'b option).
fmap is defined as a subset of ('a \<rightharpoonup> 'b) set, consisting of maps with finite domains.

In Isabelle, types must be non-empty. typedef requires you to prove that the type you are defining
is not empty.

typedef generates two functions Rep and Abs, which give correspondence between the new type and
the existing type. Rep embeds the new type into the existing type. Abs maps the corresponding
subset of the existing set to the new type.

typedef also generates lemmas about Rep and Abs, such as elimination rules (cases) and
induction rules. The injective rules state the injectivity of each of Abs (when restricted to 
the finite domain subset) and Rep. The inverse rules state that the compositions of
Rep and Abs are identity, i.e., Abs o Rep = id, and Rep o Abs is also identity when restricted
to the finite domain subset.

In the definition of fmap here, Rep is renamed to "lookup" using the morphisms keyword.
...
lemma lookup_ext: 
  ...
  (* TODO *)
  apply (simp add: lookup_inject[symmetric])
  apply (rule ext)
  by auto
...
lemma fdom_filter[simp]: "fdom (fmap_filter P m) = Set.filter P (fdom m)"
  (* TODO *)
  apply (clarsimp simp: fmap_filter_def Set.filter_def fdom_def dom_def
             split: if_splits)
...
lemma fmap_of_simps[simp]:
  ...
  (* TODO *)
   apply (simp add: fmap_of_def fempty_def)+
  apply (rule lookup_ext)
...
(* 1-e *)
lemma map_optioin_fmap_finite[simp]: "finite (dom (\<lambda>a. map_option (f a) (lookup m a)))"
  apply (rule_tac B="dom (lookup m)" in finite_subset)
    apply (simp add: dom_def)
   apply simp
  done    
...
lemma fpred_fmap_keys[simp]: "fpred P (fmap_keys f m) = fpred (\<lambda>a b. P a (f a b)) m"
  (* TODO *)
  apply (clarsimp simp: fpred_def fmap_keys_def split: option.splits)
  done
